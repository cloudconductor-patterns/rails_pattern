...
describe EventHandler do
  ...
  describe '#initialize' do
    it 'creates and returns new instance' do
      ...
      expect(event_handler.pattern_name).to eq('rails_pattern')
      expect(event_handler.pattern_dir).to eq('/opt/cloudconductor/patterns/rails_pattern')
  ...
  describe '#execute_chef' do
    it 'executes chef' do
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/roles/web_setup.json'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/roles/ap_setup.json'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/roles/db_setup.json'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
  ...
  describe '#execute_serverspec' do
    it 'executes serverspec in configure phase' do
      ...
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/web/web_configure_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/ap/ap_configure_spec.rb'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/db/db_configure_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/all/all_configure_spec.rb'
      ).and_return(false)
      event_handler.send(:execute_serverspec)
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[web,configure]',
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[db,configure]'
        ]
      )
    end
    ...
    it 'executes serverspec in deploy phase' do
      ...
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/web/web_deploy_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/ap/ap_deploy_spec.rb'
      ).and_return(false)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/db/db_deploy_spec.rb'
      ).and_return(true)
      allow(File).to receive(:exist?).with(
        '/opt/cloudconductor/patterns/rails_pattern/serverspec/spec/all/all_deploy_spec.rb'
      ).and_return(false)
      event_handler.send(:execute_serverspec)
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[web,configure]',
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[web,deploy]',
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[db,configure]',
          'cd /opt/cloudconductor/patterns/rails_pattern/serverspec; rake spec[db,deploy]'
        ]
      )
    end
  end
  ...
  describe '#create_chefsolo_config_file' do
    it 'creates config file' do
      ...
      allow(dummy_file).to receive(:write).with("role_path '/opt/cloudconductor/patterns/rails_pattern/roles'\n")
  ...
  describe '#run_chefsolo' do
    it 'runs chef-solo' do
      ...
      expect(event_handler.command).to eq(
        [
          'cd /opt/cloudconductor/patterns/rails_pattern; berks vendor ./cookbooks',
          'chef-solo -c /opt/cloudconductor/patterns/rails_pattern/solo.rb -j /opt/cloudconductor/patterns/rails_pattern/node.json'
        ]
      )
    end
  end
