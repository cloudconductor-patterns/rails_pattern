...
describe CloudConductorPattern::EventHandler do
  ...
  describe '#initialize' do
    it 'creates and returns new instance' do
      ...
      expect(event_handler.instance_variable_get('@pattern_name')).to eq('rails_pattern')
  ...
  describe '#execute_serverspec' do
    it 'executes serverspec in configure phase' do
      ...
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[web,configure]").and_return(true)
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[db,configure]").and_return(true)
    ...
    it 'executes serverspec in deploy phase' do
      ...
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[web,configure]").and_return(true)
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[web,deploy]").and_return(true)
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[db,configure]").and_return(true)
      expect(event_handler).to receive(:system).with("cd #{pattern_path}/serverspec; rake spec[db,deploy]").and_return(true)
  ...
  describe '#create_chefsolo_config_file' do
    it 'creates config file' do
      ...
      allow(dummy_file).to receive(:write).with("log_location '#{pattern_path}/logs/rails_pattern_web_chef-solo.log'\n")
