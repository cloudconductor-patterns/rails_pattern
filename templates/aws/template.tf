...
resource "aws_instance" "ap_server" {
  ...
  key_name = "${var.key_name}"
  vpc_security_group_ids = ["${aws_security_group.ap_security_group.id}", "${var.shared_security_group}"]
  subnet_id = "${element(split(", ", var.subnet_ids), 0)}"
  associate_public_ip_address = true
  tags {
    Name = "APServer"
...
resource "aws_instance" "db_server" {
  ...
  key_name = "${var.key_name}"
  vpc_security_group_ids = ["${aws_security_group.db_security_group.id}", "${var.shared_security_group}"]
  subnet_id = "${element(split(", ", var.subnet_ids), 0)}"
  associate_public_ip_address = true
  tags {
    Name = "DBServer"
